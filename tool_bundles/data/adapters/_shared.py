...
from lumen.tools.structured_data_tools import build_chart_svg, cluster_payload, correlation_payload, extract_points_from_records, load_records, regression_payload, summarize_records


def load_dataset_or_error(*, request: ToolRequest) -> tuple[list[dict[str, Any]], dict[str, Any] | None]:
    ...
    return [], {
        ...
        "failure_reason": "Need a CSV, TSV, or JSON dataset via attachment or structured records.",
...
def build_data_result(
    *,
    ...
    if svg_name and svg_content:
        svg_artifact = write_svg_artifact(outputs_dir, svg_name, svg_content)
        artifacts.append(Artifact(name=svg_name, path=svg_artifact, media_type="image/svg+xml"))
