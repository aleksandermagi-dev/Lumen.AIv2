...
class AnhToolBundle(ToolBundle):
    """Thin facade over the preserved ANH spectral analysis baseline."""

    def __init__(self, manifest: BundleManifest, repo_root: Path):
        super().__init__(manifest)
        self.repo_root = repo_root
        self._capabilities = {
            "spectral_dip_scan": ANHSpectralDipScanAdapter(manifest=manifest, repo_root=repo_root),
        }
...
def create_bundle(manifest: BundleManifest, repo_root: Path) -> ToolBundle:
    return AnhToolBundle(manifest=manifest, repo_root=repo_root)
