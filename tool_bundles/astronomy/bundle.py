...
class AstronomyToolBundle(ToolBundle):
    def __init__(self, manifest: BundleManifest, repo_root: Path):
        super().__init__(manifest)
        self._capabilities = {
            "orbit_profile": OrbitProfileAdapter(manifest=manifest, repo_root=repo_root),
        }
...
def create_bundle(manifest: BundleManifest, repo_root: Path) -> ToolBundle:
    return AstronomyToolBundle(manifest=manifest, repo_root=repo_root)
