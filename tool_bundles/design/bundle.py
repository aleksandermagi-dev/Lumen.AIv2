...
from tool_bundles.design.adapters.system_spec_adapter import SystemSpecAdapter


class DesignToolBundle(ToolBundle):
    def __init__(self, manifest: BundleManifest, repo_root: Path):
        super().__init__(manifest)
        self._capabilities = {
            "system_spec": SystemSpecAdapter(manifest=manifest, repo_root=repo_root),
        }
...
def create_bundle(manifest: BundleManifest, repo_root: Path) -> ToolBundle:
    return DesignToolBundle(manifest=manifest, repo_root=repo_root)
