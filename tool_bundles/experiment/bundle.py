from __future__ import annotations

from pathlib import Path

from lumen.tools.base import ToolBundle
from lumen.tools.registry_types import BundleManifest, ToolRequest, ToolResult

from tool_bundles.experiment.adapters.analysis_plan_adapter import AnalysisPlanAdapter
from tool_bundles.experiment.adapters.controls_adapter import ControlsAdapter
from tool_bundles.experiment.adapters.design_adapter import DesignAdapter
from tool_bundles.experiment.adapters.variables_adapter import VariablesAdapter


class ExperimentToolBundle(ToolBundle):
    def __init__(self, manifest: BundleManifest, repo_root: Path):
        super().__init__(manifest)
        self._capabilities = {
            "design": DesignAdapter(manifest=manifest, repo_root=repo_root),
            "variables": VariablesAdapter(manifest=manifest, repo_root=repo_root),
            "controls": ControlsAdapter(manifest=manifest, repo_root=repo_root),
            "analysis_plan": AnalysisPlanAdapter(manifest=manifest, repo_root=repo_root),
        }

    def execute(self, request: ToolRequest) -> ToolResult:
        if request.capability not in self._capabilities:
            raise KeyError(f"Capability '{request.capability}' is not available in bundle '{self.id}'")
        return self._capabilities[request.capability].execute(request)


def create_bundle(manifest: BundleManifest, repo_root: Path) -> ToolBundle:
    return ExperimentToolBundle(manifest=manifest, repo_root=repo_root)
