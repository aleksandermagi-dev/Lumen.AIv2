...
from lumen.tools.experiment_tools import experiment_controls_payload, infer_topic
...
class ControlsAdapter:
    ...
    def execute(self, request: ToolRequest) -> ToolResult:
        ...
        payload = experiment_controls_payload(params)
        return build_experiment_result(
            repo_root=self.repo_root,
            request=request,
            payload=payload,
            summary=f"Outlined experiment controls for {infer_topic(params)}",
            json_name="experiment_controls.json",
        )
