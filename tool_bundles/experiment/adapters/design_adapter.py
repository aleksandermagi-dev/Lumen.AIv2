...
from lumen.tools.experiment_tools import experiment_design_payload, infer_topic
...
class DesignAdapter:
    ...
    def execute(self, request: ToolRequest) -> ToolResult:
        ...
        payload = experiment_design_payload(params)
        return build_experiment_result(
            repo_root=self.repo_root,
            request=request,
            payload=payload,
            summary=f"Designed an experiment for {infer_topic(params)}",
            json_name="experiment_design.json",
        )
