...
from tool_bundles.memory.adapters.session_timeline_adapter import SessionTimelineAdapter


class MemoryToolBundle(ToolBundle):
    """Thin facade for local memory-oriented capabilities."""

    def __init__(self, manifest: BundleManifest, repo_root: Path):
        super().__init__(manifest)
        self.repo_root = repo_root
        self._capabilities = {
            "session.timeline": SessionTimelineAdapter(manifest=manifest, repo_root=repo_root),
        }
...
def create_bundle(manifest: BundleManifest, repo_root: Path) -> ToolBundle:
    return MemoryToolBundle(manifest=manifest, repo_root=repo_root)
