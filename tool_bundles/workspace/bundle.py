...
from tool_bundles.workspace.adapters.workspace_inspect_adapter import WorkspaceInspectAdapter


class WorkspaceToolBundle(ToolBundle):
    """Thin facade for local workspace inspection capabilities."""

    def __init__(self, manifest: BundleManifest, repo_root: Path):
        super().__init__(manifest)
        self.repo_root = repo_root
        self._capabilities = {
            "inspect.structure": WorkspaceInspectAdapter(manifest=manifest, repo_root=repo_root),
        }
...
def create_bundle(manifest: BundleManifest, repo_root: Path) -> ToolBundle:
    return WorkspaceToolBundle(manifest=manifest, repo_root=repo_root)
