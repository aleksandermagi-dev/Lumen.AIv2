...
# --------- user input ---------
# The preserved baseline wrapper calls the public functions in this module and
# provides the input path explicitly, so this is intentionally unset by default.
FILENAME = None
# ------------------------------
...
def inspect_spectrum_structure(path):
    with fits.open(path) as hdul:
        if len(hdul) < 2 or getattr(hdul[1], "data", None) is None:
            return {
                "valid": False,
                "recognized_format": "fits_missing_table",
                "reasons": ["FITS file does not expose a primary spectral table at HDU 1."],
            }
        names = getattr(hdul[1].data, "names", None) or []
        normalized = {str(name).upper() for name in names}
        if "WAVELENGTH" not in normalized or "FLUX" not in normalized:
            return {
                "valid": False,
                "recognized_format": "fits_missing_spectral_columns",
                "reasons": ["FITS table is missing WAVELENGTH and/or FLUX columns."],
            }
    return {
        "valid": True,
...
def analyze_siiv_targets(path, target_lines=None):
    w0, f0 = load_spectrum(path)
    line_results = []
    for line in list(target_lines or DEFAULT_TARGET_LINES):
        result = zoom(
            w0,
            f0,
            float(line["rest_wavelength"]),
            width=float(line.get("width", 1.0)),
            title=str(line.get("title") or f"Line @ {line['rest_wavelength']}"),
            smooth_win=int(line.get("smooth_window", 5)),
        )
        normalized = None
        if isinstance(result, tuple) and len(result) == 3:
            normalized = {
                "lambda_min": float(result[0]),
                "depth": float(result[1]),
                "velocity_kms": float(result[2]),
            }
        line_results.append(
            {
                "title": str(line.get("title") or f"Line @ {line['rest_wavelength']}"),
                ...
                "width": float(line.get("width", 1.0)),
                "smooth_window": int(line.get("smooth_window", 5)),
    ...
    return {
        ...
        "strongest_candidate": ranked[0] if ranked else None,
        "line_detected": bool(ranked),
    }
...
def create_overview_figure(w, f, smooth_window=11, title="HST Spectrum"):
    fig = plt.figure(figsize=(11,4))
    plt.plot(w, f, lw=0.4, label="raw")
    plt.plot(w, smooth(f, smooth_window), lw=1.0, label="smoothed")
...
def plot_si_iv_window(w, f, *, show=True):
    lo, hi = 1391.0, 1404.0
    m = (w >= lo) & (w <= hi)
    if m.sum() == 0:
...
# 2) Quick overview plot (close it to continue)
    create_overview_figure(w0, f0, smooth_window=11, title="HST Spectrum")
    plt.show()
    ...
    plot_si_iv_window(w0, f0, show=True)
